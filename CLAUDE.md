...
- **Pin every dependency to an exact version - no `^` or `~` in any package.json.** `.npmrc` sets `save-exact=true` so `pnpm add` does this automatically; if you ever hand-edit a version, match it exactly (check `pnpm-lock.yaml`'s `version:` field for that package, not the old `specifier:`). This applies to every `dependencies`/`devDependencies`/`peerDependencies` entry except `workspace:*` links, which aren't semver ranges to begin with.
